...
async function main() {
    ...
    const api = new API(process.env.SLACK_TOKEN, 'josh9051');
    ...
    async function saveItems(type){
        while (db.progress.pagination[type].current <= db.progress.pagination[type].total) {
            console.log("Pagination %s Progress %s of %s", type, db.progress.pagination[type].current, db.progress.pagination[type].total);
        ...
        while (itemIndex < db.db[type].length) {
            ...
            try {
                if(type === "messages"){
                    ...
                    await web.files.delete({file: item.id}).catch(e => console.error(JSON.stringify(e, null, 2)));
                }
            } catch (e){
