...
class DB {
    ...
    addMessageBatch(messages) {
        messages.forEach(message => {
            if (this.db.messages.findIndex(dbMessage => dbMessage.iid === message.iid) >= 0) {
                return;
            }
    ...
    async addFileBatch(files) {
        for (let file of files) {
           ...
           /* if(file.groups.length > 0 || file.ims.length > 0){
            ...
            fs.writeFileSync(path.join(this.attachmentsFolderPath, `${file.id}.${file.name.split('.').pop()}`), fileData)
        }
    }

    addBatch(items, type){
        return type === "messages" ? this.addMessageBatch(items) : this.addFileBatch(items);
    }
}
